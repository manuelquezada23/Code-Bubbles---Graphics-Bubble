...
public class GraphicsMain {
	...
	private GraphicsFill fill_shape;
		...
		graphics_pane.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent keyEvent) {
				...
				if (keyEvent.getCode() == KeyCode.BACK_SPACE) {
					if (graphics_shape != null && graphics_text == null && graphics_image == null) {
					...
					else if (graphics_shape == null && graphics_text != null && graphics_image == null) {
		...
		public void handle(MouseEvent event) {
			...
			if (current_state == GraphicsOption.drawrectangle && event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 2) {
				if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
					...
					System.out.println(this.toRGBCode(current_color));
			...
			if (event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 1) {
				...
				if (graphics_image != null && graphics_image.getNode() != null && graphics_image.getNode().contains(event.getX(), event.getY()) == true && !(event.isControlDown()) && !(event.isShiftDown())) {
					if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
				...
				if (!(graphics_shape == null) && graphics_shape.getNode().contains(event.getX(), event.getY()) == true && !(event.isControlDown() && !(event.isShiftDown()))) {					
					if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
						previous_point = new Point2D(event.getX(), event.getY());
						previous_pointX = event.getX();
						previous_pointY = event.getY();
	...
	public void Fill() {
		if (graphics_shape != null) {
			fill_shape = new GraphicsFill(graphics_shape, current_color);
